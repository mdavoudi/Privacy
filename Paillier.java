...
/**
* @ClassName: Paillier
* @Description: This is a sample java source code of Paillier PKE.
*/
public class Paillier {
	/**
	* @ClassName: PublicKey
	* @Description: This is a class for storing the public key (n, g) of Paillier PKE.
	*/
	...
	*/
	public static BigInteger encrypt(BigInteger m,PublicKey pubkey) throws Exception {
		...
		if (!belongToZStarN(m, n)) {
			throw new Exception("Paillier.encrypt(BigInteger m, PublicKey pubkey): plaintext m is not in Z*_n");
		}
	...
	*/
	...
	public static void main(String[] args) {
		...
		paillier.keyGeneration(120);
		Paillier.PublicKey pubkey = paillier.getPubkey();
		//System.out.println("pubkey "+ pubkey);
		Paillier.PrivateKey prikey = paillier.getPrikey();
		// Encryption and Decryption
		
		//BigInteger m = new BigInteger(new String("83").getBytes());
		BigInteger m = new BigInteger("8311");
		BigInteger c = null;
		...
		System.out.println("m1+m2 "+m1plusm2);
		try {
			BigInteger c1 = Paillier.encrypt(m1, pubkey);
			BigInteger c2 = Paillier.encrypt(m2, pubkey);
			//System.out.println("Enc(m1)= "+c1 +"\nEnc(m2)= "+c2);
			BigInteger c1mulc2 = Paillier.add(c1, c2, pubkey);
			//System.out.println("c1.c2= "+c1mulc2);
			BigInteger decrypted_c1mulc2 = Paillier.decrypt(c1mulc2, pubkey, prikey);
			System.out.println("Dec(c1.c2)= " +decrypted_c1mulc2 );
		...
		System.out.println("m1*m2= "+m1mulm2);
		try {
			BigInteger c1 = Paillier.encrypt(m1, pubkey);
			BigInteger c1expm2 = Paillier.mul(c1, m2, pubkey);
			BigInteger decrypted_c1expm2 = Paillier.decrypt(c1expm2, pubkey, prikey);
			System.out.println("Dec(E(m1)^m2)= "+decrypted_c1expm2);
		...
		try {
			BigInteger c1 = Paillier.encrypt(m1, pubkey);
			BigInteger c1mulrn = Paillier.selfBlind(c1, r2, pubkey);
			System.out.println("c1*r2^n mod n^2= "+c1mulrn);
			BigInteger decrypted_c1mulrn = Paillier.decrypt(c1mulrn, pubkey, prikey);
			System.out.println("Dec(c1*r2^n mod n^2)= "+decrypted_c1mulrn);
